...
This workflow publishes an update and then uploads the generated sourcemaps to an error tracking service (PostHog in this example). Publishing an update creates sourcemaps in the **dist** directory. These files only exist on the worker that runs the job, so the upload must happen in an `after_update` hook rather than in a separate job. Hook steps inherit the job's environment, so you can provide credentials such as `POSTHOG_CLI_API_KEY` as [EAS environment variables](/eas/environment-variables/).
